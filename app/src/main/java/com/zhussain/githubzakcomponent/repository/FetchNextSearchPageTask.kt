...
import com.zhussain.githubzakcomponent.api.*
...
class FetchNextSearchPageTask constructor(
    ...
    override fun run() {
        val current = db.getRepoDao().findSearchResult(query)
        if (current == null) {
        ...
        val newValue = try {
            ...
            when (val apiResponse = ApiResponse.create(response)) {
                is ApiSuccessResponse -> {
                    ...
                    db.runInTransaction {
                        db.getRepoDao().insert(merged)
                        db.getRepoDao().insertRepos(apiResponse.body.items)
                    }
